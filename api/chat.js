export default async function handler(req, res) {
    ...
    // 🚀 BULLETPROOF FIX: Using 'gemini-1.5-flash-latest' and 'v1beta'
    ...
    try {
        ...
        if (!response.ok) {
            // This logs the exact error from Google to your Vercel logs
            console.error("Google API Error:", data);
            return res.status(500).json({ 
                reply: "System busy. Error: " + (data.error?.message || "Internal API Issue") 
            });
        }
